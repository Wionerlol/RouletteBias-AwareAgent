...
详细部署流程见 [`docs/DEPLOY.md`](docs/DEPLOY.md)。

### 环境变量

| 变量 | 来源 | 说明 |
|------|------|------|
| `DATABASE_URL` | Railway Postgres 插件（自动注入）| PostgreSQL 连接字符串 |
| `API_KEY` | 手动设置 | HTTP 请求 Header `X-API-Key` 的校验值 |
| `ANTHROPIC_API_KEY` | 手动设置 | Claude API 密钥 |
...
| 方法 | 路径 | 说明 | Auth |
|------|------|------|------|
| `GET` | `/health` | 健康检查，返回 `{"status":"ok","db":"connected"}` | 无 |
| `POST` | `/session/new` | 创建 session，返回初始策略 | X-API-Key |
| `POST` | `/session/{id}/spin` | 上报开奖结果，结算上轮，返回新策略 | X-API-Key |
| `GET` | `/session/{id}/state` | 查看当前 session 全状态 | X-API-Key |
...
# 创建 session
SESSION=$(curl -sX POST $BASE/session/new \
  -H "Content-Type: application/json" -H "X-API-Key: $KEY" \
  -d '{"bankroll":800,"bet_unit":5,"recent_history":[17,5,17,32,17,11,17,5,17]}' \
  | python3 -c "import sys,json; print(json.load(sys.stdin)['session_id'])")

# 上报开奖
curl -X POST $BASE/session/$SESSION/spin \
  -H "Content-Type: application/json" -H "X-API-Key: $KEY" \
  -d '{"result_number": 17}'
```

> ⚠️ **成本提醒**：每次 `/spin` 调用约消耗 $0.01–0.05（claude-sonnet-4-6）。Railway 免费额度 $5/月，超出后需升级 Hobby 方案（$20/月）。请在 [Anthropic Console](https://console.anthropic.com) 设置消费上限。
